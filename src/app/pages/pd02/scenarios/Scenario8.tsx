...
const Scenario8 = () => {
  const scenario = PHISHING_QUIZ.find((s) => s.id === 'scn-08');
  ...
  const indicators = [
    'Dominio consistente con el servicio oficial',
    'Uso de HTTPS',
    'No solicita credenciales por correo',
    'No exige responder con datos sensibles',
    'Recomienda revisar la actividad desde el portal oficial',
  ];
  ...
  const result = useMemo<ScenarioResult | null>(() => {
    ...
    if (selectedAnswer === 'B') {
      return {
        ...
        message:
          'Correcto. Este mensaje puede considerarse legítimo porque mantiene un dominio coherente, usa HTTPS, no solicita credenciales por correo y recomienda revisar la actividad desde el portal oficial.',
      };
    }
    ...
    if (selectedAnswer === 'C') {
      return {
        ...
        message:
          'Tu respuesta refleja cautela, lo cual es positivo. Sin embargo, en este caso el mensaje presenta señales compatibles con una notificación legítima del servicio.',
      };
    }
    ...
    return {
      ...
      message:
        'En este escenario no hay señales claras de phishing. El mensaje evita pedir credenciales, usa un dominio coherente y dirige al usuario a revisar su actividad desde el portal oficial.',
    };
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <div className="bg-[#1a1a1a]/40 border border-[#d4a574]/20 p-5 space-y-2">
            <p className="text-sm font-mono text-[#d4a574]/80">
              <span className="text-[#cc6633]">Título:</span> Notificación legítima de actividad de inicio de sesión
            </p>
            <p className="text-sm font-mono text-[#d4a574]/80">
              <span className="text-[#cc6633]">Tipo de vector:</span> Notificación de seguridad
            </p>
            <p className="text-sm font-mono text-[#d4a574]/80">
              <span className="text-[#cc6633]">Categoría:</span> Aviso legítimo de autenticación
            </p>
            <p className="text-sm font-mono text-[#d4a574]/80">
        ...
        <div className="mb-8">
          ...
          <div className="text-[#d4a574]/85 font-mono leading-relaxed bg-[#1a1a1a]/40 border border-[#d4a574]/20 p-5">
            Recibes una notificación automática de seguridad sobre un inicio de sesión reciente.
            El mensaje no solicita credenciales y te indica revisar la actividad directamente desde
            el portal oficial si no reconoces el acceso.
          </div>
        </div>
        ...
        <div className="mb-10">
          ...
          <SimulatedEmailCard
            from="Seguridad de Cuenta <alertas@cuenta-oficial.com>"
            subject="Aviso de inicio de sesión reciente"
            body={`Detectamos un inicio de sesión reciente en tu cuenta.

Si reconoces esta actividad, no es necesario realizar ninguna acción.

Si no reconoces el acceso, revisa tu actividad de seguridad iniciando sesión directamente en el portal oficial.

Este mensaje es informativo y no solicita credenciales por correo.

Equipo de Seguridad`}
            linkText="Ir al portal oficial"
            linkUrl="https://cuenta-oficial.com/security/activity"
        ...
        <div className="mb-10 bg-[#1a1a1a]/40 border border-[#d4a574]/20 p-5">
          ...
          >
            ...
            <div>
              <label
                htmlFor="question-8-justification"
                className="block text-sm font-mono text-[#d4a574] mb-2"
              >
                Justificación breve (opcional)
              </label>
              <textarea
                id="question-8-justification"
                name="question-8-justification"
                rows={4}
